...
func TestNewUTXOTransaction(t *testing.T) {
	...
	assert.Equal(t, txid, tx.ID, "newutxotransaction fails!")
	...
	assert.Equal(t, vintxid, tx.Vin[0].Txid, "newutxotransaction fails!")
	assert.Equal(t, 1, tx.Vin[0].Vout, "newutxotransaction fails!")
	...
	assert.Equal(t, vinpbk, tx.Vin[0].PubKey, "newutxotransaction fails!")
	assert.Equal(t, 209999, tx.Vout[0].Value, "newutxotransaction fails!")
	pbk := []byte{0x42, 0xef, 0x7b, 0x8c, 0x33, 0xdb, 0x9f, 0x3f, 0x12, 0x4b, 0x9e, 0xb8, 0x21, 0xad, 0xd1, 0x65, 0x16, 0x34, 0xff, 0xfe}
	assert.Equal(t, pbk, tx.Vout[0].PubKeyHash, "newutxotransaction fails!")
}
